...
class OdlUtil:
	...
	def install_flow(self, container_name='default',username="admin", password="admin"):
		...
		#先把s1和s3的流表都下了
		response, content = http.request(uri='http://127.0.0.1:8181/restconf/config/opendaylight-inventory:nodes/node/openflow:1/flow-node-inventory:table/0/flow/0', body=s1_h2_To_h1, method='PUT',headers=headers)
		response, content = http.request(uri='http://127.0.0.1:8181/restconf/config/opendaylight-inventory:nodes/node/openflow:1/flow-node-inventory:table/0/flow/1', body=s1_h3_To_h1, method='PUT',headers=headers)
		...
		while True :
			...
			if speed !=0 :
				if speed < 10000 :
					...
					response, content = http.request(uri='http://127.0.0.1:8181/restconf/config/opendaylight-inventory:nodes/node/openflow:2/flow-node-inventory:table/0/flow/2', body=h3_to_s2_2_full, method='PUT',headers=headers)
				#在检测到s2的1口流量满载时发的流表
				else :
					print('s2端口1满载，h3数据包改为往2口通过')
					response, content = http.request(uri='http://127.0.0.1:8181/restconf/config/opendaylight-inventory:nodes/node/openflow:2/flow-node-inventory:table/0/flow/1', body=h3_to_s2_1_full, method='PUT',headers=headers)
